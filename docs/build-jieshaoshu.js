...
bodyChildren.push(body("标书快反（BidPilot）是一款面向工程机械经销商的投标文件 AI 智能体（v1.6）。使用者只需粘贴两段文本——本公司资料库（资质、业绩、设备参数）与招标公告原文——系统即可在 10 秒内生成投标所需的四份核心成果：资格自查表、技术偏离表、废标风险 TOP5 与投标就绪度评分报告，将传统流程中 2–3 天的人工标书处理工作压缩到数秒，并以结构化方式提示每一处可能废标的风险。"));
...
bodyChildren.push(bullet("防错：", "16 类技术参数逐条比对、★关键参数废标判定、保证金/解密时限等高风险条款自动提醒；"));
...
bodyChildren.push(body("资格自查：逐条对照招标公告的资格要求（独立法人、授权书、体系认证、业绩、联合体条款、资格审查方式等），输出“有据/待人工核实”双色标注，并自动列出保证金、CA 证书等平台类待办。技术偏离表：对 16 类技术参数逐条响应，标注满足/正偏离/负偏离/资料库无/待人工补充五态；废标风险 TOP5：以业务员可读的白话提示解密时限、同网络串标、保证金账户、授权书用印、限价与截止时间五大高危项；结论与评分：0–100 就绪度评分、A/B/C/D 等级、是否建议投标与前提条件清单。"));
...
bodyChildren.push(h2("2.7 其他功能一览"));
...
bodyChildren.push(body("引擎配套 115 项自动化回归测试（tests/engine.test.js），覆盖四个演示场景（装载机、挖掘机、起重机、泵车）、单位换算边界、★定位精确性、权重配置、HTML 报告转义防注入与乱输入鲁棒性。全量测试可通过 node tests/engine.test.js 一键复跑。"));
bodyChildren.push(h2("4.4 文件结构"));
bodyChildren.push(tbl(["文件", "职责"], [
  ["index.html", "前端应用（界面、交互、导出、历史、设备库、语言切换）"],
  ["engine.js", "核心解析引擎 v1.6（纯函数，16 类参数，公告对比，可单测）"],
  ["tests/engine.test.js", "115 项自动化回归测试"],
...
bodyChildren.push(tbl(["问题", "解答"], [
  ...
  ["支持哪些机型？", "装载机、挖掘机、起重机、泵车四大机型族 16 类参数；贴入任意含对应关键词的公告均可识别。"],
...
Packer.toBuffer(doc).then(buf => {
  fs.writeFileSync("docs/产品详细介绍书-标书快反.docx", buf);
  console.log("DOCX done: docs/产品详细介绍书-标书快反.docx");
});
