...
bodyChildren.push(C.body("本报告针对标书快反（BidPilot）V1.7的核心判定引擎（engine.js）与应用整体进行质量验证。测试目标：验证投标分析全链路（参数提取、单位换算、偏离判定、★废标识别、评分模型、公告变更检测、自定义词库、报告导出）的正确性、边界鲁棒性与反幻觉约束的有效性，为参赛演示与经销商试点交付提供质量依据。"));
bodyChildren.push(C.body("测试结论：全部115项自动化断言通过，通过率100%；浏览器人工验收覆盖主要界面路径；文档质检脚本零错误。产品满足可演示、可交付状态。"));
...
bodyChildren.push(C.tbl(["项目", "配置"], [
  ...
  ["测试对象", "engine.js v1.6.0、index.html 前端应用、sw.js PWA"],
...
bodyChildren.push(C.bullet("自动化回归测试：", "98项断言固化于 tests/engine.test.js，一条命令全量复跑（node tests/engine.test.js），任一断言失败即退出码非零、禁止发版；"));
...
bodyChildren.push(C.body("执行命令 node tests/engine.test.js，输出 PASS=115 FAIL=0，通过率100%。断言分类分布如下："));
...
bodyChildren.push(C.body("核心判定引擎在正向、反向、边界与异常输入场景下行为正确；反幻觉约束有效（无编造输出）；公告变更检测与自定义词库功能按设计工作；缺陷均已修复且纳入回归。综合评定：标书快反V1.7具备参赛演示与经销商试点交付条件。建议后续将真实招标文件纳入持续测试集，并随规则库扩充同步补充断言。"));
...
Packer.toBuffer(doc).then(buf => {
  fs.writeFileSync("docs/测试报告-标书快反.docx", buf);
  console.log("DOCX done: docs/测试报告-标书快反.docx");
});
